...
/*
 * 解密
 * static const uint32_t TD1[256]  右移1位
 * static const uint32_t TD1[256]  右移2位
 * static const uint32_t TD1[256]  右移3位
 * 
 **/
...
/*
 * LOAD32H func：将y的4个 byte 转换为一个 word output：x
 ...
 *  function：由输入的密钥生成轮秘钥
 *  Key 输入的密钥
 *  KeySize 输入的密钥大小
 *  AES128  密钥长度128 分组长度 128  Nr迭代次数10  Nk密钥字数4 输入4字(一个字16字节 128位) 输出44字 (176字节)  轮秘钥数 = 轮数 + 1 = 11
 *  AES256  密钥长度256 分组长度 128  Nr迭代次数14  Nk密钥字数4 输入4字(一个字16字节 128位) 输出60字 (480字节)  轮秘钥数 = 轮数 + 1 = 15 
 *  uint32_t        dK[60]; 数组定义60个大小的原因
 *  AES192  密钥长度192 分组长度 128  Nr迭代次数12  Nk密钥字数4 输入4字(一个字16字节 128位) 输出52字 (416字节)  轮秘钥数 = 轮数 + 1 = 13
 *
**/
    ...
    )
{
	...
	/*
	 * SETUP_MIX( Value )的功能是行移位操作和字节代换操作，等价于Subword(RotWord(temp))  生成轮密钥时循环左移一个字节
	**/
    #define SETUP_MIX( Value ) \
        ( (Te4_3[BYTE(Value, 2)]) ^ (Te4_2[BYTE(Value, 1)]) ^ (Te4_1[BYTE(Value, 0)]) ^ (Te4_0[BYTE(Value, 3)]) )

    Context->Nr = 10 + ((KeySize/8)-2)*2; //define the Encry Nr 128-10 192-12 256-14

    // Setup the forward key
    i  = 0;
    rk = Context->eK;

	//LOAD32H func：获得首个轮钥  rk[0],rk[1],rk[2],rk[3]
    LOAD32H( rk[0], key );
    LOAD32H( rk[1], key + 4 );
    LOAD32H( rk[2], key + 8 );
    LOAD32H( rk[3], key + 12 );
...
void AesEncrypt(
        ...
        uint8_t const       Input [AES_BLOCK_SIZE],     // [in] 128位待加密的states
    ...
    )
{
    ...
    rk = Context->eK; //rk 指向加密轮钥

    // Map BYTE array block to cipher state and add initial round key:将BYTE数组块添加到密码状态并添加初始轮密钥
    //输入内容放入到s0 s1 s2 s3
    LOAD32H( s0, Input );       
    LOAD32H( s1, Input + 4);
    LOAD32H( s2, Input + 8);
